function [xstar, lambda] = Newton_Monotone_LL_e(y, a, sigma, alpha, lambda, M, tol, L)
% This aims to find the root of T(lambda) = 0 whenever s = 0;
% where T(lambda) = sigma - a^T*x; 
% and x = sign(tmp)*min(max(abs(tmp) - (1/(alpha*L)), 0), M); 
...
gamma = 1/(alpha*L);
tmp = y -  lambda*gamma*a; 
xstar = sign(tmp).*min(max(abs(tmp)-gamma,0),M); 
g = sigma - sum(a.*xstar); %T(lambda)
normg = abs(g);
