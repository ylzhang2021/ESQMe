...
We next recall some important definitions that will be used in the sequel. We start by recalling the following constraint qualification for \eqref{eq1} (which first appeared in \cite{Ausleder13}), and the (associated) first-order optimality conditions for \eqref{eq1}.
...
The convergence properties of our algorithm will be studied in Section~\ref{sec4}, and we end this section by presenting some useful facts concerning the subproblem \eqref{eq2}.
...
For \eqref{eq1}, the $RCQ(x)$ holds at every point $x\in C\cap \mathcal{F}$, and for every $x\in C\setminus \mathcal{F}$, there cannot exist $u_i$, $i\in I(x)$, such that
...
\begin{enumerate}[{\rm (i)}]
  \item From the definition of $RCQ(x)$, we have that if Assumption~\ref{A1} holds, then for any $x\in C$, there cannot exist $u_i$, $i\in I(x)$, such that \eqref{A11} holds.
  \item One can show that if the $RCQ(x)$ holds at every $x\in C$, then Assumption~\ref{A1} holds.
\end{enumerate}
\end{remark}
\todo[inline]{Add some comments / concrete discussions concerning this assumption?}
...
Consider \eqref{eq1}. Suppose that Assumption \ref{A1} holds and $\bar{\beta}: = \sup_k\beta_k < \sqrt{\frac{L_g}{L_g + \ell_g}}$. Let $\{(x^k,y^k,\theta_k)\}$ be the sequence generated by Algorithm \ref{alg:Framwork}. Then any accumulation point of $\{x^k\}$ is a critical point of \eqref{eq1}.
...
\end{equation}
where we also invoked the closedness of $\partial P_1$, $\partial P_2$ and $\mathcal{N}_C$ to deduce \eqref{critical3}.

Let $\hat{\lambda}_i := \theta_{N_0}\bar{\lambda}_i\geq 0$, for all $i\in I_0\cap\{1,2,\cdots,m\}$, and $\hat{\lambda}_i = 0$, for all $i\in \{1,2,\cdots,m\}\setminus I_0$. Then by \eqref{critical1} and $I_0\subseteq \tilde{I}(\bar{x})$ (see \eqref{I0}), we have that
...
Consider \eqref{eq1}. Suppose that Assumption~\ref{A1} holds and $\bar{\beta}: = \sup_k\beta_k < \sqrt{\frac{L_g}{L_g + \ell_g}}$. Let $\{(x^k,y^k)\}$ be the sequence generated by Algorithm \ref{alg:Framwork}, and $\Omega$ be the set of accumulation points of $\{(x^{k+1},x^{k},y^{k})\}$. Then $\Omega$ is a nonempty compact set, $\omega := \lim_{k\to \infty} H(x^{k+1},x^{k}, y^{k})$ exists, and $H \equiv \omega$ on $\Omega$.
\end{lemma}
\begin{proof}
...
Consider \eqref{eq1}. Suppose that Assumptions~\ref{A1} and \ref{A2} hold, and $\bar{\beta}: = \sup_k\beta_k < \sqrt{\frac{L_g}{L_g + \ell_g}}$. Let $\{(x^k,y^k)\}$ be the sequence generated by Algorithm \ref{alg:Framwork}. Then there exist $\tau > 0$ and a positive integer $N_1$ such that for all $k \ge N_1$, we have
\[
...
\begin{align*}
& \d^2\left(0,\partial H(x^{k+1},x^{k},y^{k})\right) \\
...
F_{\hat\theta}(x) := \frac{1}{\hat\theta}(P_1(x) - \hat m) + \delta_C(x) + \max_{i = 1,\cdots,m}\left[g_i(x)\right]_+,
\end{equation}
where $\hat\theta$ is defined as in \eqref{defH} and $\hat{m} = \min\{P_1(x): x\in C\}$. Compared with $H$, the explicit KL exponent is more readily obtainable, as we will discuss in Section~\ref{sec5}.
\begin{theorem}[Convergence rate of Algorithm \ref{alg:Framwork} in convex setting]\label{Edecres}
Consider \eqref{eq1}. Suppose that Assumptions~\ref{B1} and \ref{B2} hold, and $\bar{\beta}: = \sup_k\beta_k < 1$. Let $\{(x^k,\theta_k)\}$ be the sequence generated by Algorithm \ref{alg:Framwork}. Then the following statements hold:
\begin{enumerate}[{\rm (i)}]
    \item For any $k\ge 1$,
          $$E(x^{k+1},x^{k},\theta_{k+1}) \leq E(x^k,x^{k-1},\theta_k) - \frac{(1 - \beta_k^2)L_g}{2}\|x^{k} - x^{k-1}\|^2,$$
          where $E(x,y,\theta):=\frac{1}{\theta}\left(P_1(x) - \hat{m} + \theta\max_{i = 1,\cdots,m}\left[g_i(x)\right]_+ + \frac{\theta L_g}{2}\| x - y \|^2\right)$, $\hat m$ is defined as in \eqref{definfalpha} and $L_g$ is defined in Remark~\ref{Remarkg}.
    ...
    \item If $F_{\hat\theta}$ in \eqref{definfalpha} is a KL function with exponent $\frac{1}{2}$, then $\{x^k\}$ converges to a minimizer $x^*$ of \eqref{eq1}, and there exist $c_0 > 0$, $s\in (0,1)$ and a positive integer $k_0$ such that
        \[
...
Since $C$ is compact and each $g_i$ is continuous, the feasible set ${\cal G}$ of \eqref{eq1} is compact. Combining this with the fact that $P_1$ is continuous, we have $S:= \Argmin_{x\in \mathcal{G}} P_1(x) = \Argmin F_{\hat{\theta}}(x) \not=\emptyset$. Let $\Lambda$ be the set of accumulation points of $\{x^k\}$ for notational simplicity. From Theorem \ref{subconver}(ii) and \cite[Theorem~28.3]{Ro70}, we have
\begin{equation}\label{set}
\emptyset\not=\Lambda\subseteq S.
\end{equation}

Write $E_{\theta}(x, y) := E(x, y, \theta)$ for notational simplicity. By the definition of $F_{\hat{\theta}}(x)$ in \eqref{definfalpha} and $E(x, y, \theta)$ in (i), we see that $E_{\hat{\theta}}(x, y) = F_{\hat{\theta}}(x) + \frac{L_g}{2}\|x - y\|^2$. From Remark~\ref{rem43}, Theorem~\ref{alpha} and item (i), we have that for any $k\geq N_0$, it holds that $\theta_k = \hat\theta$ and
...
Since $F_{\hat\theta}$ is convex, in view of \eqref{set}, we have $F_{\hat\theta}(\bar x) = \inf F_{\hat\theta}$ for any $\bar x\in S$. Using this together with item (ii) and the definition of $E_{\hat\theta}$, one can show readily that whenever $\bar{x}\in S$
...
\d^2((x, y), \bar{S})\leq c_0(E_{\hat{\theta}}(x, y) - \bar\omega),
\end{equation}
...
Next, notice that $\{(x^k, x^{k-1})\}\subset C\times C$ and $ \dom \partial E_{\hat{\theta}} = C\times C$. From Remark~\ref{rem43}, Theorem~\ref{subconver}(ii) and \eqref{set2}, we deduce that there exists $k_1>0$ such that
...
\d^2(x^{k}, S)\leq\d^2((x^k, x^{k-1}), \bar{S})\leq c_0(E_{\hat{\theta}}(x^k, x^{k-1}) - \bar\omega).
\end{equation}
...
&\overset{\rm(f)}\leq \bar\omega + \frac{L_g}{2\gamma}\d^2(x^k, S) + \frac{L_g}{2(1 - \gamma)}\|x^k - y^k\|^2 - \frac{L_g}{2}\d^2(x^{k+1}, S),
\end{align}
...
&\overset{\rm(a)}\leq \frac{L_g}{2\gamma}\d^2(x^k, S) + \frac{L_g}{2(1 - \gamma)}\|x^k - y^k\|^2 - \frac{L_g}{2\gamma}\d^2(x^{k+1}, S)  \nonumber\\
&~~~~+ \frac{L_g}{2}\left(\frac{1}{\gamma} - 1\right)\d^2(x^{k+1}, S) + \frac{L_g}{2}\|x^{k+1} - x^k\|^2 \\
&\overset{\rm(b)}\leq \left(\frac{L_g}{2\gamma}\d^2(x^k, S) - \frac{L_g}{2}\|x^k - x^{k-1}\|^2\right) - \left(\frac{L_g}{2\gamma}\d^2(x^{k+1}, S) - \frac{L_g}{2}\|x^{k+1} - x^k\|^2\right) \nonumber\\
...
Denote $\zeta: = \frac{1 + \bar{\beta}^2 - \gamma}{(1-\gamma)(1 - \bar{\beta}^2)} > 1$ and $A_k := \frac{L_g}{2}(\frac{1}{\gamma}\d^2(x^k, S) - \|x^k - x^{k-1}\|^2)$. Rearranging terms in the above inequality, we obtain that for any $k\ge k_4$,
\begin{equation*}
...
&\overset{\rm(a)}= M\varrho^{k} \!-\! \frac{L_g}{2(\vartheta + \zeta)}\left(\!\frac{1}{\gamma}\d^2(x^k, S) - \|x^k - x^{k-1}\|^2\!\right)\!\leq\! M\varrho^{k} \!+\! \frac{L_g}{2(\vartheta + \zeta)} \|x^k - x^{k-1}\|^2\notag\\
...
which implies that $\{x^k\}$ is a Cauchy sequence. Combining this with Remark~\ref{rem43} and Theorem~\ref{subconver}(ii), we see that $\{x^k\}$ converges to a minimizer $x^*$ of \eqref{eq1}. The claimed linear convergence rate also follows immediately from \eqref{bound}.
\end{proof}
...
\min_{x\in \R^n}F(x):= P_1(x) + \delta_C(x) + \delta_{{\cal F}}(x),
\end{equation}
...
\end{equation}
where $\eta > 0$. Notice that the KL property of $\hat F_{\hat\theta}$ (or, equivalently, that of $F_{\hat\theta}$) was the key for establishing the convergence rate of the $\{x^k\}$ generated by Algorithm~\ref{alg:Framwork}.
...
\end{definition}
\todo[inline]{End of edits here, as of Oct 24, 2023.}
\begin{remark}
  We first argue that the set of exact penalty parameters of \eqref{KLproblem} is nonvoid.
\end{remark}

Now, we show that if $F$ in \eqref{KLproblem} is KL function with exponent $\alpha$, then
the $\hat F_{\eta}$ is a KL function with exponent $\alpha$, for any $\eta>\bar{\eta}$, where $\bar{\eta}$ is an exact penalty parameter of \eqref{KLproblem}.

\begin{theorem}\label{FetaKL}
Let $F$ be as in \eqref{KLproblem}, $\bar{x}\in\Argmin F$ and $\bar{\eta}$ be the exact penalty parameter of \eqref{KLproblem}. If $F$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$, then for any $\eta>\bar{\eta}$, $\hat F_{\eta}(x)$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$.
\end{theorem}

\todo{Add a remark discussing the condition $\eta > \bar \eta$.}

\begin{proof}
%In view of \cite[Lemma~2.1]{li18} and the convexity of $\hat F_{\eta}$, it suffices to show that $\hat F_{\eta}$ has a KL property at  $\bar{x}\in\{x : 0\in\partial \hat F_{\eta}(x)\} = \Argmin \hat F_{\eta}$ with exponent $\alpha$.
...
and $F(\bar{x}) \leq F(x) \leq F(\bar{x}) + a$.

Since $\bar{\eta}$ is the exact penalty parameter of \eqref{KLproblem}, we see that for any $\eta\geq \bar{\eta}$, $\Argmin F = \Argmin \hat F_{\eta}$ and $\dom\partial \hat F_{\eta} = C$.
...
Taking $\epsilon_0 := \min\{\epsilon, \bar{\epsilon}, \frac{a}{3L_{P_1}}\}$, then for any $\eta\geq\bar{\eta}$, for any $x\in C = \dom\partial \hat F_{\eta}$ satisfying $\|x - \bar{x}\|\leq \epsilon_0$, we have that
...
Letting $a_0 := a - 2\epsilon_0 L_{P_1}>0$. Then for any $\eta\geq\bar{\eta}$, for any $x\in C = \dom\partial \hat F_{\eta}$ satisfying $\hat F_{\eta}(\bar{x}) \leq \hat F_{\eta}(x) \leq \hat F_{\eta}(\bar{x}) + a_0$, we claim that
...
\[
P_1(x) \leq P_1(x) + \eta\max_i[g_i(x)]_+ = \hat F_{\eta}(x)\leq \hat F_{\eta}(\bar{x}) + a_0 = P_1(\bar{x}) + a - 2\epsilon_0 L_{P_1}.
\]
...
Hence, for any $\eta>\bar{\eta}$, for any $x\in C = \dom\partial \hat F_{\eta}$ satisfying $\|x - \bar{x}\|\leq \epsilon_0$ and $\hat F_{\eta}(\bar{x}) \leq \hat F_{\eta}(x) \leq \hat F_{\eta}(\bar{x}) + a_0$, we have that
...
Specifically, we consider the following constrained optimization problem:
\begin{equation}\label{KLproblem2}
...
From \cite[Thoerem~5.1]{zhang23}, using Theorem~\ref{FetaKL}, we have that under some suitable conditions, $\hat F_{\eta}$ satisfies the KL property, for any $\eta>\bar{\eta}$, where $\bar{\eta}$ is the exact penalty parameter of \eqref{KLproblem}.
...
\end{enumerate}
Then for any $\eta>\bar{\eta}$, $\hat F_{\eta}$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$.
\end{corollary}
...
\end{enumerate}
Then, for any $\eta>\bar{\eta}$, $\hat F_{\eta}$ is KL function with exponent $\alpha$.
\end{corollary}
...
We use FISTA (which reset parameters with fixed and adaptive restart) to generate the sequence $\{\beta_k\}$. In more detail, set the initial values $\vartheta_{-1}=\vartheta_{0}=1$, define for $k \ge 0$,
\begin{align}\label{beta}
\beta_k = \frac{\vartheta_{k-1}-1}{\vartheta_{k}},
\end{align}
