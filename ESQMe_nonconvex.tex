...
\end{align*}
for any $x\in C$, where (a) holds because both $\hat x$ and $\tilde x$ minimize $P_1$ over ${C\cap \mathscr{F}}$, (b) holds because $\hat x$ also minimizes $\bar\eta F_{\bar\eta}$, and (c) holds because $\eta> \hat\eta$. As for the converse inclusion, let $\tilde x \in \Argmin F_{\eta}\cap \mathscr{F}$. Then for any $x\in C\cap \mathscr{F}$, we have
\[
