...
(ii): Note that the objective in the subproblem \eqref{eq2} is strongly convex in $x$. Using this, and noting the definition of $(x^{k+1},s^{k+1})$ in \eqref{eq2} as a minimizer and the fact that $(x^k,\max_{i = 1,\cdots,m}\left[\ling{i}{x^{k}}{y^k}\right]_+)$ is feasible for \eqref{eq2}, we have for any $k\ge 0$ that
\begin{equation}
