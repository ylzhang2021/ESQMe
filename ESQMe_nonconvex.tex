...
In our numerical tests, we let $\mu =0.95$ in \eqref{CompSen1-1} and $(q,n,k) = (720i,2560i,160i)$ with $i\in \{2, 4, 6, 8, 10\}$. For each $i$, we generate 20 random instances as described above. We present the computational results when $\epsilon$ in \eqref{termination} equals $10^{-4}$ and $10^{-6}$ in Table~\ref{table1} and Table~\ref{table2}, respectively, averaged over the $20$ random instances. Here, we present the time for computing the QR decomposition of $A^T$ (denoted by $t_{\rm QR}$), the time for computing $\|A\|^2$ (denoted by $t_{\|A\|}$),\footnote{We compute $\|A\|^2$ via the Matlab code {\sf norm(A*A')} when $p \le 2000$, and is computed using {\sf eigs(A*A',1,'LM')} otherwise.} the time for computing $x^0 = A^\dagger b$ given the QR factorization of $A^T$ (denoted by $t_{A^\dagger b}$),\footnote{Note that $A^\dagger b$ is only used by SCP$_{\rm ls}$, and $\|A\|$ is only used by ESQM$_{\rm b}$ and ESQM$_{\rm e}$.} the CPU times of the algorithms (CPU time), the number of iterations (denoted by Iter), the recovery errors $\text{RecErr} := \frac{\|x^* - \xorig\|}{\max\{1, \|\xorig\|\}}$ and the residuals ${\rm Residual} := \frac{\|Ax^* - b\|^2 - \sigma^2_1 }{\sigma^2_1}$, where $x^*$ is the approximate solution returned by the respective algorithm.
...
&  2.601 &  8.950 & 19.348 & 31.669 & 47.208           \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{\rm b}$}
...
& -3.60e-05 & -4.50e-05 & -2.33e-05 & -3.16e-05 & -3.33e-05      \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
...
&  3.095 & 10.169 & 22.397 & 37.676 & 57.152            \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{\rm b}$}
...
& -5.29e-09 & -6.76e-09 & -7.02e-09 & -6.18e-09 & -1.08e-08      \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
...
&  1.929 & 10.603 & 12.760 & 76.295 & 46.565         \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{\rm b}$}
...
&    171 &    295 &    156 &    560 &    228         \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{\rm b}$}
...
&  0.082 &  0.086 &  0.086 &  0.087 &  0.089       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
...
& -2.71e-07 & -2.79e-07 & -4.08e-07 & -2.84e-07 & -3.28e-07  \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
