...
We compare SCP$_{\rm ls}$ in \cite{yu21}, ESQM (i.e., setting $\beta_k \equiv 0$ in Algorithm~\ref{alg:Framwork}), ESQM$_{\rm ls}$\footnote{This is a variant of the ESQM with line search described in \cite{Ausleder13}. Here, we implement the adaptation in \cite[Algorithm~3]{zhang23}.} and ESQM$_{\rm e}$ (i.e., Algorithm~\ref{alg:Framwork}). We use the same parameter settings for SCP$_{\rm ls}$ in \cite{yu21}, and the initial point of SCP$_{\rm ls}$ is chosen as $x^0 = A^\dagger b$. In ESQM and ESQM$_{\rm e}$, we take $L_g = \|A\|^2$, $\ell_g = 0$, $d = 1$ and $\theta_0 = 1$, and we use the same parameter settings for ESQM$_{\rm ls}$ as described on \cite[Page~31]{zhang23}; for these three algorithms, the initial points are chosen as $x^0 = 0$. We terminate all algorithms when $\|x^{k+1} - x^k\|< \epsilon\cdot\max\{1, \|x^{k+1}\|\}$ for some $\epsilon > 0$ specified below. In addition, for ESQM$_{\rm ls}$, we also terminate when the $t_k$ given in \cite[Algorithm~3]{zhang23} is small as described in \cite[Page~32]{zhang23} (specifically, when $t_k\le 10^{-8}$). The subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
...
As in the previous subsection, we compare SCP$_{\rm ls}$, ESQM, ESQM$_{\rm ls}$ and ESQM$_{\rm e}$. For SCP$_{\rm ls}$, we use the same parameter settings in \cite{yu21}, and initialize it at $x^0 = A^\dagger b$. In ESQM and ESQM$_{\rm e}$, we take $L_g = \frac{2\|A\|}{\gamma^2}$, $\ell_g = \frac{\|A\|}{4\gamma^2}$, $d = \frac{\gamma^2}{20}$ and $\theta_0 = 0.05$, and we use the same parameter settings for ESQM$_{\rm ls}$ as described on \cite[Page~31]{zhang23}; these three algorithms are initialized at $x^0 = 0$. We terminate all algorithms when $\|x^{k+1} - x^k\|< \epsilon\cdot\max\{1, \|x^{k+1}\|\}$ for some $\epsilon > 0$ specified below. For ESQM$_{\rm ls}$, we also terminate it when the $t_k$ given in \cite[Algorithm~3]{zhang23} is small as described in \cite[Page~32]{zhang23} (specifically, when $t_k\le 10^{-8}$). Furthermore, the subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
