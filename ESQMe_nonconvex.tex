...
\author[3]{\fnm{Xiqi} \sur{Xu}}\email{xxx.com}
%\equalcont{These authors contributed equally to this work.}
...
\begin{align*}
0&\!\in \partial P_1(x^{k+1}) \!- \xi^k \!+ \theta_k\partial\max\limits_{i = 0, 1,\cdots,m}\{\ling{i}{\cdot}{y^k}\}(x^{k+1}) \!+ \theta_kL_g(x^{k+1} - y^k) \!+ \mathcal{N}_C(x^{k+1})\\
...
H(x,y,z) := \frac{P(x) - \bar{m}}{\hat{\theta}} + \delta_{C}(x) + \max\limits_i[\ling{i}{x}{z}\rangle]_+ + \frac{L_g}{2}\| x-y \|^2 + \frac{L_g}{2}\| x - z \|^2,
\end{equation}
...
\left[
\begin{array}{c}
\frac{1}{\hat{\theta}}\widehat{\partial} P(x^{k+1}) + \conv_{i\in I_k(x^{k+1})}\{\nabla g_i(y^k)\} + \widehat{\mathcal{N}}_C(x^{k+1}) + L_g(x^{k+1} - x^k) + L_g(x^{k+1} - y^k)    \\
 - L_g(x^{k+1} - x^{k}) \\
...
\right],
\end{align}
where (a) holds because of \cite[Proposition~10.5, Corollary~10.9, Exercise~8.31]{rock97a}, (b) follows from $\widehat{\partial} P(x^{k+1}) = \widehat{\partial} P_1(x^{k+1}) - \nabla P_2(x^{k+1}) = \partial P_1(x^{k+1}) - \nabla P_2(x^{k+1}) = \partial P(x^{k+1})$ and $\widehat{\mathcal{N}}_C(x^{k+1}) = \mathcal{N}_C(x^{k+1})$ (thanks to \cite[Corollary~10.9, Proposition~8.12, Exercise~8.8]{rock97a}), and (c) holds because $\sum\limits_{i\in I_k(x^{k+1})} \lambda_i^k = 1$, where $I_k(x^{k+1})$ and $\lambda_i^k$ were defined as in Lemma~\ref{subproremarks}(iii).
