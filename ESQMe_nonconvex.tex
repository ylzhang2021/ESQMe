...
{\color{red} Plot:
\begin{enumerate}
  \item Review / discuss the use of extrapolation in gradient projection methods, i.e., deal with sets that are easy to project onto.
  \item Mention that we consider incorporating extrapolation techniques for empirically accelerating an algorithm for a class of difference-of-convex optimization problems whose constraint sets are not easy to project onto. State the problem formulation, and mention its use. Then state the algorithm we study and cite some references.
  \item Mention some competing algorithms, and mention that our ESQM$_e$ outperforms them.
  \item Describe structure together with contributions.
\end{enumerate}
}
