...
The convergence properties of our algorithm will be studied in Section~\ref{sec4}, and we end this section by presenting some useful facts concerning the subproblem \eqref{eq2}. The first two items are trivial observations noted here for easy reference later.
...
\begin{lemma}\label{lem43}
  ...
  $\bar\eta > 0$ is such that $\Argmin F_{\bar\eta}\cap \Argmin_{x\in\cal G}P_1(x)\neq \emptyset$, then $\Argmin_{x\in {\cal G}}P_1(x) = \Argmin F_{\eta}$ whenever $\eta > \bar\eta$.
...
Let $F$ be as in \eqref{KLproblem}, $\bar{x}\in\Argmin F$ and $\bar{\eta}$ be an exact penalty parameter of \eqref{KLproblem}. If $F$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$, then for any $\eta>\bar{\eta}$, $\hat F_{\eta}(x)$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$.
\end{theorem}

\begin{proof}
Fix any $\eta > \bar\eta$. Since $\bar{\eta}$ is an exact penalty parameter of \eqref{KLproblem}, we see that $\Argmin F = \Argmin \hat F_{\eta}$; also, note that $\dom\partial \hat F_{\eta} = C$.

Since $F$ satisfies the KL property with exponent $\alpha$ at $\bar{x}$ and $C$ is compact, by Lemma~\ref{KLinequ}, there exist $c>0$ and $a$, $\epsilon\in (0,1)$ such that
\begin{equation}\label{FKL}
\d(x, \Argmin F) \leq c(F(x) - F(\bar{x}))^{\alpha},
\end{equation}
whenever $ x\in\dom \partial F = C\cap \mathcal{F}$ satisfies $\|x - \bar{x}\|\leq \epsilon$ %$\d(x, \Argmin F)\leq\epsilon$
...
\]
Combining this with \eqref{upperproje}, one can see that $P_1(\Pi_{C\cap \mathcal{F}}(x))\leq P_1(\bar{x}) + a$. Combining this with the definition of $F$, we conclude that \eqref{projecx} holds.
...
&\overset{\rm(b)}\leq c(F(\Pi_{C\cap \mathcal{F}} (x)) - F(\bar{x}))^{\alpha} + \kappa\d(x, \mathcal{F}) = c(P_1(\Pi_{C\cap \mathcal{F}} (x)) - P_1(\bar{x}))^{\alpha} + \kappa\d(x, \mathcal{F}) \notag\\
...
\begin{remark}[On the condition $\eta > \bar \eta$ in Theorem~\ref{FetaKL}]\label{rem52}
  We comment on the usefulness of Theorem~\ref{FetaKL}, which only infers the KL exponent of $\hat F_\eta$ when $\eta > \bar \eta$ for some exact penalty parameter $\bar\eta$. Specifically, consider \eqref{eq1} and suppose that Assumptions~\ref{B1} and \ref{B2} hold. Then we deduce by combining \eqref{conditionunhappy} and Lemma~\ref{lem43} that the set of exact penalty parameters contains the interval $(\hat\theta,\infty)$. If it happens that there is an exact penalty parameter less than $\hat\theta$, then the condition \eqref{assumptionunhappy} holds (thanks to Lemma~\ref{lem43}) and Theorem~\ref{FetaKL} asserts that the KL property required in Theorem~\ref{Edecres}(iii) can be inferred from that of $P_1+\delta_C+\delta_{\mathscr{F}}$ in \eqref{eq1}.
\end{remark}

\todo{End of edit here.}
...
\end{aligned}
\end{equation}
where $M: = (1 - \mu)^{-1}\Big(\|\tilde{x}\|_1 - \mu\|\tilde{x}\|\Big)$, where $\tilde{x}:= A^{\dagger}b$ is feasible for \eqref{CompSen}.

Now, problem \eqref{CompSen1} is a special case of \eqref{eq1} with $P_1(x): = \|x\|_1$, $P_2(x) := \mu\|x\|$, $g(x) := h(Ax - b)$ and $C: = \{x: \|x\|_{\infty}\leq M\}$. Since $A$ has full row rank and $h(0) = 0< \sigma$, we see that $\tilde x \in C\cap \{x: g(x) < 0 \}\not=\emptyset$.
...
\newblock {\em SIAM J. Optim.} 26, 1465--1492, 2016.
