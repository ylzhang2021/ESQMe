...
We compare SCP$_{\rm ls}$ in \cite{yu21}, ESQM (i.e., setting $\beta_k \equiv 0$ in Algorithm~\ref{alg:Framwork}), ESQM$_{\rm ls}$ in \cite{Ausleder13} (which is an adaptation of the ESQM with line search) and ESQM$_{\rm e}$ (i.e., Algorithm~\ref{alg:Framwork}). We use the same parameter settings for SCP$_{\rm ls}$ in \cite{yu21}, and the initial point of SCP$_{\rm ls}$ is chosen as $x^0 = A^\dagger b$. In ESQM, ESQM$_{\rm ls}$ and ESQM$_{\rm e}$, we take $L_g = \|A\|^2$, $\ell_g = 0$, $d = 1$ and $\theta_0 = 1$, and the initial points are chosen as $x^0 = 0$. We terminate all algorithms when $\|x^{k+1} - x^k\|< \epsilon\cdot\max\{1, \|x^{k+1}\|\}$. Moreover, as a safeguard, we also terminate when stepsize is small for ESQM$_{\rm ls}$. The subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
...
As in the previous subsection, we compare SCP$_{\rm ls}$ in \cite{yu21}, ESQM (i.e., setting $\beta_k \equiv 0$ in Algorithm~\ref{alg:Framwork}), ESQM$_{\rm ls}$ in \cite{Ausleder13} (which is an adaptation of the ESQM with line search) and ESQM$_{\rm e}$ (i.e., Algorithm~\ref{alg:Framwork}). We use the same parameter settings for SCP$_{\rm ls}$ in \cite{yu21}, and the initial point of SCP$_{\rm ls}$ is chosen as $x^0 = A^\dagger b$. In ESQM, ESQM$_{\rm ls}$ and ESQM$_{\rm e}$, we take $L_g = \frac{2\|A\|}{\gamma^2}$, $\ell_g = \frac{\|A\|}{4\gamma^2}$, $d = \frac{\gamma^2}{20}$ and $\theta_0 = 0.05$, and the initial points are chosen as $x^0 = 0$. We terminate all algorithms when $\|x^{k+1} - x^k\|< \epsilon\cdot\max\{1, \|x^{k+1}\|\}$. Moreover, as a safeguard, we also terminate when stepsize is small for ESQM$_{\rm ls}$. Furthermore, the subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
