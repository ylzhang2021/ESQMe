...
\end{equation}
We identify our algorithm as an extended sequential quadratic method with extrapolation (ESQM$_{\text{e}}$), where ``extrapolation" refers to step \eqref{defyk}. This is because when $\beta_k \equiv 0$, our algorithm reduces to an instance of the ESQM proposed in \cite{Ausleder13}, whose convergence was established for solving \eqref{eq1} when the $P$ in \eqref{eq1} is in addition smooth with Lipschitz gradient.\footnote{More precisely, when the $P$ in \eqref{eq1} is smooth with Lipschitz gradient (say, with Lipschitz constant $L_P$) and $\beta_k \equiv 0$, our algorithm applied to \eqref{eq1} with $P_1(x) := \frac{L_P}2\|x\|^2$ and $P_2(x) := \frac{L_P}2\|x\|^2 - P(x)$ becomes an instance of the ESQM in \cite{Ausleder13}.} Notice that $(x^{k+1},s^{k+1})$ is the minimizer of the subproblem in \eqref{eq2} if and only if $s^{k+1} = \max_{i = 1,\cdots,m}[\ling{i}{x^{k+1}}{y^k}]_+$ and
\begin{equation}\label{subproblem2}
x^{k+1} \in \Argmin\limits_{x\in C}~ P_1(x) - \langle \xi^k, x \rangle + \theta_k \max_{i = 0, 1,\cdots,m}\{\ling{i}{x}{y^k}\} + \frac{\theta_k L_g}{2}\| x - y^k \|^2.
\end{equation}
...
We now prove (iii). Note that $x^{k+1}$ is a component of the minimizer of the subproblem in \eqref{eq2} if and only if it is a minimizer of the convex problem \eqref{subproblem2}. Using $g_0\equiv 0$ and \cite[Theorem 23.8]{Ro70}, this is further equivalent to
\begin{align*}
...
\end{lemma}
\begin{proof}
Let $\varLambda$ be the set of accumulation points of $\{x^k\}$. Then $\varLambda$ is nonempty and compact in view of Theorem~\ref{suffdec}(i), and $\varLambda\subseteq {\cal X}$ thanks to Theorem~\ref{subconver}. Moreover, we have $\lim_{k\to \infty}\d(x^k,\varLambda)=0$. Since $\Lambda\subseteq {\cal X}\subset U_0$ (where $U_0$ is defined as in Assumption \ref{A2}) and $\Lambda$ is compact, there exists a bounded open set $U_1$ and an $N_2\in \mathbb{N}$ such that $x^k\in U_1$ for all $k\geq N_2$ and the closure of $U_1$ is contained in $U_0$.\footnote{The existence of such a $U_1$ can be argued as follows. Since $\Lambda$ is compact, there exists $\epsilon > 0$ such that $\{x:\; \d(x,\Lambda) < \epsilon\}\subseteq U_0$. Then set $U_1 := \{x:\; \d(x,\Lambda) < \epsilon/2\}$.}
...
\end{align*}
where (a) holds because $\hat x\in {C\cap \mathscr{F}}$ (hence $\max_{i=1,\ldots,m}[g_i(\hat x)]_+ = 0$) and (b) holds because $\hat x$ also minimizes $\bar\eta F_{\bar\eta}$. Rearranging terms in the above inequality, we obtain $(\eta-\bar\eta)\max_{i=1,\ldots,m}[g_i(\tilde x)]_+ = 0$, which means $\tilde x\in \mathscr{F}$.
\end{proof}
...
Next, notice that $\{(x^k, x^{k-1})\}\subseteq C\times C\subset \dom \partial E_{\hat{\theta}} = C\times \R^n$, and we have from Theorem~\ref{suffdec}(iii) that $\bar{\Lambda}$ is the set of accumulation points of the bounded sequence $\{(x^k,x^{k-1})\}$. Using this and \eqref{set2}, we deduce that there exists $k_1\in \mathbb{N}$ such that
...
\begin{proof}
  ...
  We then apply Lemma~\ref{RobEB} with $\Omega:=\mathcal{F}$, $g(x) = (g_1(x), g_2(x), \ldots, g_m(x))$, $x^s = \hat{x}$ and $\delta_0 = \left|\max_i{g_i(\hat{x})}\right|$ to obtain
\begin{equation*}
...
In our numerical tests, we let $\mu =0.95$ in \eqref{CompSen1-1} and $(q,n,k) = (720i,2560i,160i)$ with $i\in \{2, 4, 6, 8, 10\}$. For each $i$, we generate 20 random instances as described above. We present the computational results when $\epsilon$ in \eqref{termination} equals $10^{-4}$ and $10^{-6}$ in Table~\ref{table1} and Table~\ref{table2}, respectively, averaged over the $20$ random instances. Here, we present the time for computing the QR decomposition of $A^T$ (denoted by $t_{\rm QR}$), the time for computing $\|A\|^2$ (denoted by $t_{\|A\|}$),\footnote{The $\|A\|^2$ is computed via the Matlab code {\sf norm(A*A')} when $p \le 2000$, and is computed using {\sf eigs(A*A',1,'LM')} otherwise.} the time for computing $x^0 = A^\dagger b$ given the QR factorization of $A^T$ (denoted by $t_{A^\dagger b}$),\footnote{Note that $A^\dagger b$ is only used by SCP$_{\rm ls}$, and $\|A\|$ is only used by ESQM$_{\rm b}$ and ESQM$_{\rm e}$.} the CPU times of the algorithms in seconds (CPU time), the number of iterations (denoted by Iter), the recovery errors $\text{RecErr} := \frac{\|x^* - \xorig\|}{\max\{1, \|\xorig\|\}}$ and the residuals ${\rm Residual} := \frac{\|Ax^* - b\|^2 - \sigma^2_1 }{\sigma^2_1}$, where $x^*$ is the approximate solution returned by the respective algorithm.
...
In our numerical tests, we let $\mu =0.95$ in \eqref{CompSen1-2} and $(q,n,k) = (720i,2560i,80i)$ with $i\in \{2, 4, 6, 8, 10\}$. For each $i$, we generate 20 random instances as described above. The computational results for $\epsilon$ in \eqref{termination} being $10^{-4}$ and $10^{-6}$ are respectively presented in Table~\ref{table3} and Table~\ref{table4}, averaged over the $20$ random instances. As before, we present the time for computing the QR decomposition of $A^T$ (denoted by $t_{\rm QR}$), the time for computing $\|A\|^2$ (denoted by $t_{\|A\|}$), the time for computing $x^0 = A^\dagger b$ given the QR factorization of $A^T$ (denoted by $t_{A^\dagger b}$), the CPU times of the algorithms in seconds (CPU time), the number of iterations (denoted by Iter), the recovery errors $\text{RecErr} := \frac{\|x^* - \xorig\|}{\max\{1, \|\xorig\|\}}$ and the residuals ${\rm Residual} := \frac{\|Ax^* - b\|_{LL_2,\gamma} - \sigma }{\sigma}$, where $x^*$ is the approximate solution returned by the respective algorithm.
...
&  2.412 & 12.479 & 14.589 & 79.060 & 52.279           \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{\rm b}$}
...
&    189 &    314 &    177 &    578 &    247           \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{\rm b}$}
...
&  0.082 &  0.086 &  0.086 &  0.086 &  0.088       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
...
& -2.51e-11 & -3.08e-11 & -4.18e-11 & -3.20e-11 & -2.97e-11  \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{\rm b}$}
