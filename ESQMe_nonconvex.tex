...
\end{equation}
We identify our algorithm as an extended sequential quadratic method with extrapolation (ESQM$_{\text{e}}$), where ``extrapolation" refers to step \eqref{defyk}. This is because when $\beta_k \equiv 0$, our algorithm reduces to an instance of the ESQM proposed in \cite{Ausleder13}, whose convergence was established for solving \eqref{eq1} when the $P$ in \eqref{eq1} is in addition smooth with Lipschitz gradient.\footnote{More precisely, when the $P$ in \eqref{eq1} is smooth with Lipschitz gradient (say, with Lipschitz constant $L_P$) and $\beta_k \equiv 0$, our algorithm applied to \eqref{eq1} with $P_1(x) := \frac{L_P}2\|x\|^2$ and $P_2(x) := \frac{L_P}2\|x\|^2 - P(x)$ becomes an instance of the ESQM in \cite{Ausleder13}.} Notice that the subproblem in \eqref{eq2} has a unique solution as an optimization problem with a nonempty closed convex feasible set and an objective strongly convex in $x$ (note that the optimal $s$ is clearly uniquely determined). While it requires an iterative solver in general, we refer the readers to \cite[Appendix~A]{zhang23} for an efficient routine for solving the subproblem in \eqref{eq2} with some specific $P_1$ when $m = 1$.
\begin{algorithm}
\caption{ESQM$_{\text{e}}$ for solving \eqref{eq1}}\label{alg:Framwork}
\begin{algorithmic}
\STATE
\begin{description}
  \item[\bf Step 0.] Choose $x^{-1}=x^0\in C$, $\theta_0>0$, $d>0$, and $\{\beta_k\}\subseteq\left[0,\sqrt{\frac{L_g}{L_g + \ell_g}}~\right)$ with $\bar\beta:= \sup_k\beta_k < \sqrt{\frac{L_g}{L_g + \ell_g}}$, where $L_g = \max\{L_{g_i}, ~ i=1,\dots,m\}$ and $\ell_g = \max\{\ell_{g_i}, ~ i=1,\dots,m\}$ as in Remark~\ref{Remarkg}.
  \item[\bf Step 1.] Set
...
\end{equation}
where $I(x) := \Big\{ s\in\{1, 2, \dots, m\}: g_s(x) = \max_{i = 1, 2, \dots, m} [g_i(x)]_+ \Big\}$.\footnote{From the definition of $I_k(\cdot)$ in \eqref{defiIk}, one can see that the difference between $I_k(\cdot)$ and $I(\cdot)$ is that $I_k(\cdot)\subseteq\{0, 1,\cdots,m\}$, while $I(\cdot)\subseteq\{1,\cdots,m\}$. We would also like to point out that while our definition of $I(x)$ seems to look slightly different from the corresponding definition, namely $T(x)$, in \cite{Ausleder13} (see the discussions before \cite[Eq.~(6)]{Ausleder13}), one can check that the two definitions are equivalent.}
\end{assumption}
...
\[
I_0 \subseteq I(x^*) =\left\{i \in\{1,2,\cdots,m\}: g_i(x^*) = \max_{i=1,\cdots,m} [g_i(x^*)]_+\right\},
\]
...
\end{equation}
where $\tilde{I}(\bar{x}):=\left\{s \in\{0,1,\cdots,m\}: g_s(\bar{x}) = \max_{i=0,1,\cdots,m} \{g_i(\bar x)\}\right\}$ and $\theta_{N_0}$ is defined in Theorem~\ref{alpha}; moreover, $\bar x$ is a critical point of \eqref{eq1}.
\end{theorem}
\begin{proof}
...
\right]\!\!,\!\!\!\!
\end{align}
where $\Xi(x,y,z) := \max_i[\ling{i}{x}{z}]_+$, and $I_k(x^{k+1})$ and $\lambda_i^k$ are defined as in Lemma~\ref{subproremarks}(iii);
%\[
%\Xi_k \!\!:=\! \frac{1}{\hat{\theta}}\partial P(x^{k+1}) + \conv\!_{i\in I_k(x^{k+1})}\{\nabla g_i(y^k)\} + \mathcal{N}_C(x^{k+1}) + L_g(x^{k+1} - x^k) + L_g(x^{k+1} - y^k),
%\]
here, (a) holds because of the subdifferential calculus rules in \cite[Proposition~10.5, Corollary~10.9]{rock97a} and the regularity of the normal cone of $C$ in \cite[Proposition~6.17]{rock97a}, (b) holds upon invoking \cite[Exercise~8.8(c)]{rock97a}, the regularity of $P_1$ (thanks to \cite[Proposition~8.12]{rock97a}) and the fact that $P_2$ is continuously differentiable at $x^{k+1}$, and (c) follows from \cite[Proposition~10.5, Exercise~8.31]{rock97a} and the fact that $\sum_{i\in I_k(x^{k+1})} \lambda_i^k = 1$ and $\lambda_i^k\ge 0$ for all $i\in I_k(x^{k+1})$.
...
Suppose that in \eqref{eq1}, $P_2 = 0$ and $g_1,\ldots, g_m$ are convex.\footnote{Under this assumption, we also set $\ell_{g_i}\equiv 0$ in Algorithm~\ref{alg:Framwork}.}
\end{assumption}
...
\end{align*}
where (a) holds because $\hat x\in {C\cap \mathscr{F}}$ and (b) holds because $\hat x$ also minimizes $\bar\eta F_{\bar\eta}$. Rearranging terms in the above inequality, we obtain $(\eta-\bar\eta)\max_{i=1,\ldots,m}[g_i(\tilde x)]_+ = 0$, which means $\tilde x\in \mathscr{F}$.
\end{proof}
...
\end{aligned}
\end{equation}
where $M: = (1 - \mu)^{-1}\Big(\|A^\dagger b\|_1 - \mu\|A^\dagger b\|\Big)$.\footnote{We also recall from \cite[Section~6.1]{zhang23} that $\|A^\dagger b\|_\infty\le M$.} Notice that the equivalent problem \eqref{CompSen1} is a special case of \eqref{eq1} with $P_1(x) = \|x\|_1$, $P_2(x) = \mu\|x\|$, $m = 1$, $g_1(x) = h(Ax - b)-\sigma$ and $C = \{x: \|x\|_{\infty}\leq M\}$. Since $A$ has full row rank and $h(0) = 0< \sigma$, we see that $A^\dagger b \in C\cap \{x: g_1(x) < 0 \}\not=\emptyset$.
...
We compare SCP$_{\rm ls}$ in \cite{yu21}, ESQM (i.e., setting $\beta_k \equiv 0$ in Algorithm~\ref{alg:Framwork}), ESQM$_{\rm ls}$\footnote{This is a variant of the ESQM with line search described in \cite{Ausleder13}. Here, we implement the adaptation in \cite[Algorithm~3]{zhang23}.} and ESQM$_{\rm e}$ (i.e., Algorithm~\ref{alg:Framwork}). We use the same parameter settings for SCP$_{\rm ls}$ in \cite{yu21}, and the initial point of SCP$_{\rm ls}$ is chosen as $x^0 = A^\dagger b$. In ESQM and ESQM$_{\rm e}$, we take $L_g = \|A\|^2$, $\ell_g = 0$, $d = 1$ and $\theta_0 = 1$, and we use the same parameter settings for ESQM$_{\rm ls}$ as described on \cite[Page~31]{zhang23} except that we set $\delta = 1$ there; for these three algorithms, the initial points are chosen as $x^0 = 0$. We terminate all algorithms when 
\begin{equation}\label{termination}
  \|x^{k+1} - x^k\|< \epsilon\cdot\max\{1, \|x^{k+1}\|\}
\end{equation}
for some $\epsilon > 0$ specified below. In addition, for ESQM$_{\rm ls}$, we also terminate when the $t_k$ given in \cite[Algorithm~3]{zhang23} is small as described in \cite[Page~32]{zhang23} (specifically, when $t_k\le 10^{-8}$). The subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
...
In our numerical tests, we let $\mu =0.95$ in \eqref{CompSen1-1} and $(q,n,k) = (720i,2560i,80i)$ with $i\in \{2, 4, 6, 8, 10\}$. For each $i$, we generate 20 random instances as described above. We present the computational results when $\epsilon$ in \eqref{termination} equals $10^{-4}$ and $10^{-6}$ in Table~\ref{table1} and Table~\ref{table2}, respectively, averaged over the $20$ random instances. Here, we present the time for computing the QR decomposition of $A^T$ (denoted by $t_{\rm QR}$), the time for computing $\|A\|$ (denoted by $t_{\|A\|}$),  the time for computing $x^0 = A^\dagger b$ given the QR factorization of $A^T$ (denoted by $t_{A^\dagger b}$),\footnote{Note that $A^\dagger b$ is only used by SCP$_{\rm ls}$, and $\|A\|$ is only used by ESQM, ESQM$_{\rm ls}$ and ESQM$_{\rm e}$.} the CPU times of the algorithms (CPU time), the number of iterations (denoted by Iter), the recovery errors $\text{RecErr} := \frac{\|x^* - \xorig\|}{\max\{1, \|\xorig\|\}}$ and the residuals ${\rm Residual} := \frac{\|Ax^* - b\| - \sigma_1 }{\sigma_1}$, where $x^*$ is the approximate solution returned by the respective algorithm.
...
\\\cline{1-7}\multirow{7}*{CPU time} & \multirow{1}*{$t_{\rm QR}$}
&  0.584 &  3.891 & 12.487 & 28.747 & 62.051          \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{A^\dagger b}$}
&  0.007 &  0.028 &  0.061 &  0.109 &  0.166          \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{\|A\|}$}
&  0.576 &  1.512 &  4.559 & 10.069 & 18.745          \\\cline{2-7} \multirow{1}*{} & \multirow{1}*{SCP$_{\rm ls}$}
&  1.017 &  3.475 &  7.379 & 12.783 & 19.159          \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM}
&  3.076 & 11.285 & 24.933 & 44.355 & 67.478          \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  1.642 &  5.961 & 13.001 & 23.030 & 37.427          \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.263 &  0.975 &  2.191 &  3.881 &  6.259          \\\cline{1-7} \multirow{4}*{Iter} & \multirow{1}*{SCP$_{\rm ls}$}
&     92 &     94 &     93 &     93 &     95          \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM}
&    516 &    533 &    533 &    542 &    540          \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm ls}}$}
&    261 &    271 &    274 &    278 &    281          \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm e}}$}
&     48 &     48 &     48 &     49 &     48          \\\cline{1-7} \multirow{4}*{RecErr} & \multirow{1}*{SCP$_{\rm ls}$}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
&  0.036 &  0.036 &  0.036 &  0.037 &  0.037       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  0.035 &  0.035 &  0.034 &  0.035 &  0.036       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{1-7} \multirow{4}*{Residual} & \multirow{1}*{SCP$_{\rm ls}$}
& -6.19e-06 & -6.19e-06 & -7.43e-06 & -1.19e-05 & -1.10e-05      \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
& 2.21e-06 & 2.16e-06 & 2.20e-06 & 2.07e-06 & 2.06e-06     \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
& 5.05e-06 & 4.96e-06 & 4.98e-06 & 4.71e-06 & 4.66e-06     \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
& 2.94e-06 & 2.94e-06 & 3.07e-06 & 3.08e-06 & 2.89e-06    \\\cline{1-7}
\end{tabular}
}
\end{center}
}
...
& \multicolumn{1}{c|}{ $i = 6$ } & \multicolumn{1}{|c|}{ $i = 8$ } & \multicolumn{1}{c|}{ $i = 10$ }
\\\cline{1-7}\multirow{7}*{CPU time} & \multirow{1}*{$t_{\rm QR}$}
&  0.528 &  3.815 & 13.598 & 31.743 & 59.845           \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{A^\dagger b}$}
&  0.006 &  0.027 &  0.062 &  0.111 &  0.173           \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{\|A\|}$}
&  0.439 &  1.437 &  4.833 & 10.567 & 19.405           \\\cline{2-7} \multirow{1}*{} & \multirow{1}*{SCP$_{\rm ls}$}
&  1.042 &  3.789 &  8.296 & 14.357 & 22.073           \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM}
&  4.358 & 17.119 & 37.869 & 67.117 & 105.270           \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  2.285 &  8.796 & 19.529 & 33.946 & 54.719           \\\cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.489 &  1.817 &  4.163 &  7.136 & 11.495           \\\cline{1-7} \multirow{4}*{Iter} & \multirow{1}*{SCP$_{\rm ls}$}
&    103 &    105 &    105 &    106 &    106           \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM}
&    756 &    786 &    782 &    798 &    798           \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm ls}}$}
&    376 &    393 &    394 &    401 &    405           \\\cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm e}}$}
&     84 &     85 &     86 &     86 &     86           \\\cline{1-7} \multirow{4}*{RecErr} & \multirow{1}*{SCP$_{\rm ls}$}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.033 &  0.033 &  0.033 &  0.034 &  0.034       \\\cline{1-7} \multirow{4}*{Residual} & \multirow{1}*{SCP$_{\rm ls}$}
& -1.28e-09 & -1.40e-09 & -9.95e-10 & -1.22e-09 & -1.56e-09      \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
& -7.56e-10 & -2.44e-10 & -1.05e-10 & -1.68e-11 & 1.80e-11     \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
& -4.81e-10 & -1.43e-11 & 1.51e-10 & 2.24e-10 & 2.54e-10     \\\cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
& -3.12e-10 & -1.42e-10 & 8.34e-11 & 4.62e-11 & -1.29e-11    \\\cline{1-7}
\end{tabular}
}
\end{center}
}
...
From Table~\ref{table1} and Table~\ref{table2}, one can see that ESQM$_{{\rm e}}$ is the fastest, and the recovery errors of all four methods are comparable.
...
\end{aligned}
\end{equation}
We first argue that Assumption~\ref{A1} holds for \eqref{CompSen1-2} under our assumptions on $A$ and $\sigma$ in \eqref{CompSen1}. To this end, let $\hat h(y) := \|y\|_{L L_2, \gamma} - \sigma$ for notational simplicity. Then \eqref{CompSen1-2} is an instance of \eqref{eq1} with $g_1(x) = \hat h(Ax - b) - \sigma$ and $C := \{x:\; \|x\|_\infty \le M\}$. Next, recall that $A^\dagger b\in C$ by construction. Moreover, observe that for any $x\in C$, we have
...
\end{proof}
In view of the above discussions, and the observation that the $H$ corresponding to \eqref{CompSen1-1} is a subanalytic function that is continuous on its closed domain (and hence a KL function in view of \cite[Theorem~3.1]{bolte07}), one can apply Theorem~\ref{th2.2} with $L_g = \frac{2\|A\|}{\gamma^2}$ and $\ell_g = \frac{\|A\|}{4\gamma^2}$ to deducing the convergence of the $\{x^k\}$ generated by Algorithm~\ref{alg:Framwork} with $\sup_{k}\beta_k < \sqrt{\frac{L_g}{L_g + \ell_g}} = \sqrt{\frac89}$ for solving \eqref{CompSen1-2}.

As in the previous subsection, we compare SCP$_{\rm ls}$, ESQM, ESQM$_{\rm ls}$ and ESQM$_{\rm e}$. For SCP$_{\rm ls}$, we use the same parameter settings in \cite{yu21}, and initialize it at $x^0 = A^\dagger b$. In ESQM and ESQM$_{\rm e}$, we take $L_g = \frac{2\|A\|}{\gamma^2}$, $\ell_g = \frac{\|A\|}{4\gamma^2}$, $d = \frac{\gamma^2}{20}$ and $\theta_0 = 0.05$, and we use the same parameter settings for ESQM$_{\rm ls}$ as described on \cite[Page~31]{zhang23} except that we set $\delta = \frac{\gamma^2}{20}$ there; these three algorithms are initialized at $x^0 = 0$. We terminate all algorithms when \eqref{termination} holds for some $\epsilon > 0$ specified below. For ESQM$_{\rm ls}$, we also terminate it when the $t_k$ given in \cite[Algorithm~3]{zhang23} is small as described in \cite[Page~32]{zhang23} (specifically, when $t_k\le 10^{-8}$). Furthermore, the subproblems in these algorithms are solved according to the procedures described in the appendices of \cite{yu21} and \cite{zhang23}.
...
In our numerical tests, we let $\mu =0.95$ in \eqref{CompSen1-2} and $(q,n,k) = (720i,2560i,80i)$ with $i\in \{2, 4, 6, 8, 10\}$. For each $i$, we generate 20 random instances as described above. The computational results for $\epsilon$ in \eqref{termination} being $10^{-4}$ and $10^{-6}$ are respectively presented in Table~\ref{table3} and Table~\ref{table4}, averaged over the $20$ random instances. As before, we present the time for computing the QR decomposition of $A^T$ (denoted by $t_{\rm QR}$), the time for computing $\|A\|$ (denoted by $t_{\|A\|}$), the time for computing $x^0 = A^\dagger b$ given the QR factorization of $A^T$ (denoted by $t_{A^\dagger b}$), the CPU times of the algorithms (CPU time), the number of iterations (denoted by Iter), the recovery errors $\text{RecErr} := \frac{\|x^* - \xorig\|}{\max\{1, \|\xorig\|\}}$ and the residuals ${\rm Residual} := \frac{\|Ax^* - b\|_{LL_2,\gamma} - \sigma }{\sigma}$, where $x^*$ is the approximate solution returned by the respective algorithm.
...
& \multicolumn{1}{c|}{ $i = 6$ } & \multicolumn{1}{|c|}{ $i = 8$ } & \multicolumn{1}{c|}{ $i = 10$ }
\\ \cline{1-7}\multirow{7}*{CPU time} & \multirow{1}*{$t_{\rm QR}$}
&  0.499 &  3.500 & 11.481 & 25.946 & 53.884        \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{A^\dagger b}$}
&  0.005 &  0.027 &  0.055 &  0.090 &  0.152        \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{\|A\|}$}
&  0.429 &  1.358 &  4.300 &  9.463 & 19.223        \\ \cline{2-7} \multirow{1}*{} & \multirow{1}*{SCP$_{\rm ls}$}
&  1.640 & 10.213 & 11.611 & 69.903 & 44.578        \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM}
&  2.611 & 10.992 & 24.816 & 45.332 & 72.512        \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  2.603 & 10.939 & 24.947 & 44.881 & 72.872        \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.538 &  2.229 &  4.977 &  9.190 & 15.136        \\ \cline{1-7} \multirow{4}*{Iter} & \multirow{1}*{SCP$_{\rm ls}$}
&    171 &    295 &    156 &    560 &    228        \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM}
&    580 &    568 &    567 &    582 &    593        \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm ls}}$}
&    580 &    568 &    566 &    582 &    593        \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm e}}$}
&    117 &    115 &    112 &    119 &    122        \\ \cline{1-7} \multirow{4}*{RecErr} & \multirow{1}*{SCP$_{\rm ls}$}
&  0.082 &  0.086 &  0.086 &  0.087 &  0.089       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
&  0.085 &  0.089 &  0.089 &  0.090 &  0.092       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  0.085 &  0.089 &  0.089 &  0.090 &  0.092       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.082 &  0.086 &  0.086 &  0.087 &  0.088       \\ \cline{1-7} \multirow{4}*{Residual} & \multirow{1}*{SCP$_{\rm ls}$}
& -2.71e-07 & -2.79e-07 & -4.08e-07 & -2.84e-07 & -3.28e-07  \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
& 1.12e-07 & 1.00e-07 & 9.78e-08 & 9.49e-08 & 9.39e-08     \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
& 1.12e-07 & 9.95e-08 & 9.78e-08 & 9.50e-08 & 9.39e-08     \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
& 6.17e-08 & 4.28e-08 & 6.89e-08 & 4.41e-08 & 2.41e-08     \\ \cline{1-7}
\end{tabular}
}
\end{center}
}
...
& \multicolumn{1}{c|}{ $i = 6$ } & \multicolumn{1}{|c|}{ $i = 8$ } & \multicolumn{1}{c|}{ $i = 10$ }
\\ \cline{1-7}\multirow{7}*{CPU time} & \multirow{1}*{$t_{\rm QR}$}
&  0.583 &  3.549 & 11.223 & 27.638 & 47.080         \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{A^\dagger b}$}
&  0.005 &  0.023 &  0.051 &  0.089 &  0.143         \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{$t_{\|A\|}$}
&  0.476 &  1.360 &  4.263 &  9.423 & 16.822         \\ \cline{2-7} \multirow{1}*{} & \multirow{1}*{SCP$_{\rm ls}$}
&  1.921 & 10.349 & 13.193 & 71.974 & 47.011         \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM}
&  3.871 & 15.987 & 36.914 & 66.225 & 107.560         \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  3.798 & 15.937 & 36.917 & 66.799 & 107.957         \\ \cline{2-2} \multirow{1}*{}  & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.723 &  2.863 &  6.638 & 11.790 & 18.965         \\ \cline{1-7} \multirow{4}*{Iter} & \multirow{1}*{SCP$_{\rm ls}$}
&    189 &    314 &    177 &    578 &    247         \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM}
&    837 &    839 &    839 &    861 &    879         \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm ls}}$}
&    836 &    839 &    839 &    861 &    879         \\ \cline{2-2} \multirow{1}*{}     & \multirow{1}*{ESQM$_{{\rm e}}$}
&    155 &    150 &    150 &    152 &    156         \\ \cline{1-7} \multirow{4}*{RecErr} & \multirow{1}*{SCP$_{\rm ls}$}
&  0.082 &  0.086 &  0.086 &  0.086 &  0.088       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
&  0.082 &  0.086 &  0.086 &  0.086 &  0.088       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
&  0.082 &  0.086 &  0.086 &  0.086 &  0.088       \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
&  0.082 &  0.086 &  0.086 &  0.086 &  0.088       \\ \cline{1-7} \multirow{4}*{Residual} & \multirow{1}*{SCP$_{\rm ls}$}
& -2.51e-11 & -3.08e-11 & -4.18e-11 & -3.20e-11 & -2.97e-11  \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM}
& 1.09e-11 & 9.74e-12 & 9.51e-12 & 9.46e-12 & 9.26e-12     \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm ls}}$}
& 1.09e-11 & 9.69e-12 & 9.49e-12 & 9.44e-12 & 9.24e-12     \\ \cline{2-2} \multirow{1}*{} & \multirow{1}*{ESQM$_{{\rm e}}$}
& 4.79e-12 & 5.00e-12 & 3.43e-12 & 5.72e-12 & 6.87e-12     \\ \cline{1-7}
\end{tabular}
}
\end{center}
}
...
From Table~\ref{table3} and Table~\ref{table4}, we observe a similar pattern as shown in Table~\ref{table1} and Table~\ref{table2}, i.e., ESQM$_{{\rm e}}$ is the fastest algorithm, and the recovery errors of all four methods are comparable.
